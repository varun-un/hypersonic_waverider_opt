        ...
        with open(log_file_path, 'r', encoding='utf-8') as file:
            for line in file:
                ...
                if current_iteration is not None and func_val_pattern.search(line):
                    # print(line)  # Optional: Keep or remove debugging print
...
def generate_vtks_for_valid_iterations(log_file_path, output_dir):
    ...
    ordered from greatest (least negative) to least (most negative) function value.
    ...
    # Sort valid iterations by function_value descending (least negative to most negative)
    valid_iterations.sort(key=lambda x: x['function_value'], reverse=True)
    print("Sorted valid iterations from least negative to most negative function value.")
    ...
    # Save function values to an array for reference (optional)
    eval_values = [it['function_value'] for it in valid_iterations]

    # Process each valid iteration
    for idx, it in enumerate(valid_iterations, start=1):
        ...
        try:
            ...
            print(f"Generated mesh for iteration {iteration_num} with function value {it['function_value']}: {filepath}")
        except Exception as e:
...
if __name__ == "__main__":
    main()
    # Example usage:
    # python generate_ordered_vtks.py trace.log valid_vtks
